...
acc <- "GSE114192"
de_file <- file.path(path_result, "02_bulk", acc, "tables", "T01_GSE114192_TBDM_vs_TB_DESeq2.csv")
if (!file.exists(de_file)) stop("Run GSE114192 primary DESeq2 analysis first")
...
ranks <- rank_dt$stat
names(ranks) <- rank_dt$gene_symbol
...
run_fgsea <- function(pathways, framework) {
  set.seed(20260807)
  x <- fgsea::fgseaMultilevel(pathways = pathways, stats = ranks, minSize = 15, maxSize = 500,
                              eps = 0, scoreType = "std")
  x <- data.table::as.data.table(x)
  x[, framework := framework]
  x[, leadingEdge_text := vapply(leadingEdge, paste, collapse = ";", character(1))]
  x[, leadingEdge := NULL]
  x
}
...
safe_write_csv(gsea, file.path(result_dir, "tables", "T01_GSE114192_Hallmark_Reactome_GSEA.csv"))

top <- gsea[order(padj, -abs_NES), head(.SD, 10), by = framework]
top[, label := factor(pathway_label, levels = rev(unique(pathway_label)))]
top[, direction := factor(ifelse(NES > 0, "Higher in TB-DM", "Lower in TB-DM"),
                          levels = c("Lower in TB-DM", "Higher in TB-DM"))]
safe_write_csv(top, file.path(result_dir, "source_data", "SD01_GSE114192_top_GSEA.csv"))

p <- ggplot2::ggplot(top, ggplot2::aes(NES, label, size = -log10(pmax(padj, 1e-300)), colour = direction)) +
  ...
  ggplot2::scale_colour_manual(values = c("Lower in TB-DM" = "#416A9A", "Higher in TB-DM" = "#B84A4A")) +
  ggplot2::scale_size_continuous(range = c(1.5, 4.5), name = expression(-log[10](FDR))) +
  ggplot2::labs(title = "Pathway-level host-response shifts in TB-diabetes",
                subtitle = "GSE114192; fgseaMultilevel on DESeq2 Wald statistics",
...
out <- file.path(result_dir, "figures", "F04_GSE114192_Hallmark_Reactome_GSEA")
...
write_log("GSE114192 Hallmark/Reactome GSEA completed: ", sum(gsea$padj < 0.05, na.rm = TRUE),
