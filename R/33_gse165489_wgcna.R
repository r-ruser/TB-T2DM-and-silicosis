...
acc <- "GSE165489"
model_file <- file.path(path_result, "02_bulk", acc, "models", "M01_GSE165489_primary_models.rds")
gene_map_file <- file.path(path_data, "02_GEO_bulk", acc, "processed", paste0(acc, "_gene_map.csv"))
if (!all(file.exists(c(model_file, gene_map_file)))) stop("GSE165489 primary model inputs missing")
primary <- readRDS(model_file)
dds <- primary$dds
meta <- data.table::as.data.table(as.data.frame(SummarizedExperiment::colData(dds)), keep.rownames = "sample_id")
...
selected <- names(sort(gene_mad[is.finite(gene_mad) & gene_mad > 0], decreasing = TRUE))[1:5000]
...
if (anyNA(meta$sample_id)) stop("GSE165489 WGCNA metadata ordering failed")
...
data.table::setnames(fit, c("Power", "SFT.R.sq", "slope", "mean.k."),
                     c("power", "scale_free_r2", "slope", "mean_connectivity"), skip_absent = TRUE)
candidate <- fit[scale_free_r2 >= 0.80 & slope < 0]
soft_power <- if (nrow(candidate)) min(candidate$power) else fit[slope < 0][which.max(scale_free_r2), power]

net <- WGCNA::blockwiseModules(dat_expr, power = soft_power, networkType = "signed",
  TOMType = "signed", corType = "pearson", minModuleSize = 30, mergeCutHeight = 0.25,
  numericLabels = TRUE, pamRespectsDendro = FALSE, maxBlockSize = 6000,
  saveTOMs = FALSE, verbose = 2)
mes <- WGCNA::orderMEs(net$MEs)
assoc <- data.table::rbindlist(lapply(names(mes), function(me_name) {
  d <- data.frame(me = mes[[me_name]], group = meta$group, exposure_z = meta$exposure_z)
  lm_fit <- stats::lm(me ~ exposure_z + group, data = d)
  cf <- summary(lm_fit)$coefficients
  cn <- grep("^groupsilicosis$", rownames(cf), value = TRUE)
  est <- cf[cn, "Estimate"]; se <- cf[cn, "Std. Error"]
  data.table::data.table(module = sub("^ME", "", me_name), eigengene = me_name,
    estimate = est, std_error = se, ci_low = est - 1.96 * se, ci_high = est + 1.96 * se,
    p_value = cf[cn, "Pr(>|t|)"])
}))
...
membership <- data.table::as.data.table(membership, keep.rownames = "entrez_id")
membership_long <- data.table::melt(membership, id.vars = "entrez_id",
  variable.name = "eigengene", value.name = "kME")
membership_long[, module := sub("^kME", "", eigengene)]
gene_map <- data.table::fread(gene_map_file)
membership_long[, gene_symbol := gene_map$gene_symbol[match(entrez_id, gene_map$entrez_id)]]
...
safe_write_csv(fit, file.path(result_dir, "source_data", "SD01_GSE165489_soft_threshold.csv"))
safe_write_csv(assoc, file.path(result_dir, "tables", "T01_GSE165489_module_silicosis_association.csv"))
safe_write_csv(membership_long, file.path(result_dir, "source_data", "SD02_GSE165489_module_membership.csv"))
saveRDS(list(network = net, eigengenes = mes, selected_power = soft_power,
             genes = colnames(dat_expr), dat_expr = dat_expr, sample_metadata = meta),
        file.path(result_dir, "models", "M01_GSE165489_WGCNA_network.rds"), compress = FALSE)
...
plot_assoc[, module_label := factor(sprintf("Module %s (n=%d)", module, module_size),
                                    levels = rev(sprintf("Module %s (n=%d)", module, module_size)))]
...
p_b <- ggplot2::ggplot(plot_assoc, ggplot2::aes(estimate, module_label, colour = evidence)) +
  ...
  ggplot2::labs(title = "Module association with silicosis",
                subtitle = "Linear model adjusted for exposure duration",
                x = "Adjusted eigengene difference", y = NULL, colour = NULL)
theme_w <- ggplot2::theme_classic(base_size = 7, base_family = "Arial") +
...
fig <- (p_a | p_b) + patchwork::plot_layout(widths = c(0.8, 1.2)) +
  patchwork::plot_annotation(tag_levels = "a", title = "GSE165489 co-expression network",
                             subtitle = "Independent signed WGCNA; 5,000 most variable genes") & theme_w
out <- file.path(result_dir, "figures", "F10_GSE165489_WGCNA")
...
write_log("GSE165489 WGCNA completed: power=", soft_power, "; modules=", nrow(assoc),
